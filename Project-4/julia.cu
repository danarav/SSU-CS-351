...
void julia(Complex d, Complex center, Color* pixels) {
    ...
    // Map pixel (x, y) into the complex plane
    Complex z(
        (x - Width / 2) * d.x + center.x,
        (y - Height / 2) * d.y + center.y
    );

    // Julia constant (same idea as CPU version)
    Complex c(-0.8f, 0.156f);

    int iterations = 0;
    while (iterations < MaxIterations && magnitude(z) < 2.0f) {
        z = z * z + c;
        iterations++;
    }

    pixels[y * Width + x] = setColor(iterations);
};
